...
public class StreamFunctionTeste {
	...
	private static void streamComparator(StreamFunctions stream) {
		...
		lista.forEach(func -> {
			System.out.println("Nomes ordenados por ordem alfabética: " + func.getNome());
		});
	}
	...
	private static void retornaOptional(StreamFunctions stream) throws Exception {
		 ...
		 funcionario.ifPresent(f -> System.out.println("Funcionário: " + f.getNome() + " " + "idade: " +f.getIdade()));
			
		 funcionario.orElseThrow(() -> new Exception("Não foi encontrado o objeto Funcionario"));
	...
	private static void retornaMapaComListaFuncionariosComMesmaIdade(StreamFunctions stream) {
		...
		lista.forEach(funcionario -> {
			 System.out.println("Idade do funcionário : " + funcionario.getNome() + " " + "é : " + funcionario.getIdade());
		});
		System.out.println("-----------------------------------------------------------------------------------------");
		
		mapa.forEach((k, v) -> {
			 System.out.println("A chave : " + k + " " + "contém uma lista de funcionários com a mesma idade");
			 //listas de funcionários com a mesma idade
			 v.forEach(f -> {
				 System.out.println("Idade do funcionário : " + f.getNome() + " " + "é : " + f.getIdade());
			 });
		});
