...
public class StreamFunctions {
	...
	public Optional<Funcionario> retornaOptional(List<Funcionario> consultarFuncionariosCadastrados) {
		...
		Optional<Funcionario> ofuncionarioptFunc = consultarFuncionariosCadastrados.stream().filter(f ->  Integer.valueOf(f.getIdade()) > 20).findFirst();
		return ofuncionarioptFunc;
	}

	public List<Funcionario> comparator(List<Funcionario> consultarFuncionariosCadastrados) {

		Stream<Funcionario> streamFunc = consultarFuncionariosCadastrados.stream().sorted(Comparator.comparing(Funcionario::getNome)); 
		return streamFunc.collect(Collectors.toList());
	}
